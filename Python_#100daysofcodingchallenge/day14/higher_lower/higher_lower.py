...
def display_battle(celebs, score):
    ...
    print(logo)
    if score > 0: print(f"Correct, Your current score is {score}.")
    for num in range(0, len(celebs)):
        if num == 1: print(vs)
        print(f"{celebs[num]['name']} a {celebs[num]['description']} from {celebs[num]['country']}")

def user_choice(celebs):
    '''Allows user to make a selection between two choices'''
    choice = ""
    while True:
        if choice != "": print(f"You must choose 'A' for {celebs[0]['name']} or 'B' for {celebs[1]['name']}")
        choice = input("Which has a greater follower count? 'A' or 'B': ").upper()
